...
static int l_entity_get(lua_State* L) {
    ...
    const auto scene = static_cast<Scene*>(lua_touserdata(L, -1));
    create_entity_table(L, &scene->entities.at(scene->script_to_uuid.at(entity)));
...
static int l_camera_set_zoom(lua_State* L) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "_scene");
    const auto scene = static_cast<Scene*>(lua_touserdata(L, -1));
    scene->camera.zoom(scale, sfu::Camera::ZoomFunc::Linear);
...
static int l_set_mode(lua_State* L) {
    ...
    const auto scene = static_cast<Scene*>(lua_touserdata(L, -1));
    switch (mode) {
    case 0: scene->set_mode(GameMode::Normal); break;
    case 1: scene->set_mode(GameMode::Dialogue); break;
    case 2: scene->set_mode(GameMode::Cinematic); break;
    case 3: scene->set_mode(GameMode::Combat); break;
    }
...
static int l_yield_combat(lua_State* L) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "_scene");
    const auto scene = static_cast<Scene*>(lua_touserdata(L, -1));
    scene->begin_combat(ally_tags, enemy_tags, {}, {});
...
static int l_yield_dialogue(lua_State* L) {
    ...
    try {
        ...
        scene->begin_dialogue(std::move(dia), filename);
...
static int l_yield_dialogue(lua_State* L) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "_scene");
    const auto scene = static_cast<Scene*>(lua_touserdata(L, -1));
    scene->begin_dialogue(shmy::speech::Graph::load_from_file(shmy::env::pkg_full() / filename), filename);
...
static int l_set_overlay(lua_State* L) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "_scene");
    const auto scene = static_cast<Scene*>(lua_touserdata(L, -1));
    scene->game->render_settings.overlay = col;
...
static int l_goto_scene(lua_State* L) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "_scene");
    const auto scene = static_cast<Scene*>(lua_touserdata(L, -1));
    scene->game->queue_scene_swap(idx, spawnpos);
    return 0;
}
