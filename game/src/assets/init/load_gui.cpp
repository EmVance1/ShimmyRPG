...
void SceneLoader::load_gui() {
    const auto viewport = game->render_settings.viewport;
    auto& gui = game->gui;

    {
        auto label = gui::Text::create(gui::Position::topcenter({0.f, 50.f}), sf::Vector2f(300, 35), game->style, game->get_active_scene().name);
        label->set_text_alignment(gui::Position::Alignment::Center);
        label->set_text_padding(0.f);
        gui.add_widget("area_label", label);
    }
    {
        auto gold = gui::Panel::create(gui::Position::topright({-50.f, 30.f}), sf::Vector2f(100, 100), game->style);
        gold->set_background_texture(sf::IntRect{ {150, 0}, {100, 100} });

        auto goldtxt = gui::Text::create(gui::Position::topleft({0.f, 70.f}), sf::Vector2f(100, 30), game->style, "100");
    ...
    {
        auto tooltip = gui::Text::create(gui::Position::topleft({0.f, 0.f}), sf::Vector2f(100, 35), game->style, "");
...

    {
        ...
        game->cinematic_mode.dia_gui = dialogue_gui;
...

    {
        ...
        game->combat_mode.atk_gui = combat_gui;
        ...
        end_turn->set_callback([&](){ game->combat_mode.advance_turn = true; });
