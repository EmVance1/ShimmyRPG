...
SpatialGraph2d SpatialGraph2d::create_grid(const sf::Vector2i& size) {
    ...
    for (int y = 0; y < size.y; y++) {
        for (int x = 0; x < size.x; x++) {
            ...
            for (int i = -1; i < 2; i++) {
                for (int j = -1; j < 2; j++) {
                    if (!(i == 0 && j == 0) && y + i >= 0 && y + i < size.y && x + j >= 0 && x + j < size.x) {
...
SpatialGraph2d load_grid_from_image(const sf::Image& img) {
    auto pathfind = SpatialGraph2d::create_grid({(int)img.getSize().x, (int)img.getSize().y});
